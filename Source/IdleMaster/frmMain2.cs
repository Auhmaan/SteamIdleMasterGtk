...
namespace IdleMaster
{
    public partial class frmMain2 : Form
    {
        ...
        private void StartIdle()
        {
            ...
            if (string.IsNullOrWhiteSpace(Settings.Default.CookieSessionId) || !IsSteamReady)
            {
                ResetClientStatus();
                return;
            }
        ...
        {
            bool connected = !string.IsNullOrWhiteSpace(Settings.Default.CookieSessionId) && !string.IsNullOrWhiteSpace(Settings.Default.CookieLoginSecure);
        ...
        private async void tmrCardDropCheck_Tick(object sender, EventArgs e)
        {
            if (TimeLeft <= 0)
            {
                ...
                //Check if user is authenticated and if any badge left to idle
                //There should be check for IsCookieReady, but property is set in timer tick, so it could take some time to be set
                tmrCardDropCheck.Enabled = !string.IsNullOrWhiteSpace(Settings.Default.CookieSessionId) && IsSteamReady && Badges.Any() && TimeLeft != 0;
            }
            else
            {
                TimeLeft = TimeLeft - 1;
